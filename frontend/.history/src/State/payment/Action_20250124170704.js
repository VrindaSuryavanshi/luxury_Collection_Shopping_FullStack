export const cretePaymentRequest = (reqData) =>
     async (dispatch) => {
        ...
        try {
            const {data} = await axios.post(`/api/payments/${reqData.orderId}`, reqData);
...
export const getPaymentRequest = (orderId) =>
    async (dispatch) => {
        ...
        try {
            const { data } = await axios.get(`/api/payments/${orderId}`);
...
export const updatePaymentRequest = (reqData) =>
    async (dispatch) => {
        ...
        try {
            const { data } = await axios.get(`/api/payments?payment_id=${reqData.orderId}&order_id=${reqData.orderId}`, reqData);

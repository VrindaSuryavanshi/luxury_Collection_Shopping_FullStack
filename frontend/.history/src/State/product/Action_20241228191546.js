...
export const findProducts = (reqData)=> async (dispatch)=>{
   ...
   const {color,size,minPrice,maxPrice,minDiscount,category,sort,stock,pageNumber,pageSize}=reqData;
    try {
        ...
        const {data} = await api.get(`/api/products?color=${color}&size=${size}&minPrice=${minPrice}&maxPrice=${maxPrice}&minDiscount=${minDiscount}&category=${category}&stock=${stock}&sort=${sort}&pageNumber=${pageNumber}&pageSize=${pageNumber}`)
    
            console.log("Product data with filter" , data );
